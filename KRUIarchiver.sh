...
do
	...
	##
	prefix="Main" #Prefix used when naming files.
	stamp=$(date "+%p" | tr '[:upper:]' '[:lower:]') # Yes, this is required because the BSD date won't give you a lowercame am/pm. 
	radiostream="http://krui.student-services.uiowa.edu:8000/listen.m3u" #Webstream to rip audio from.
	filename="$prefix--$(date "+%I-%M-%S").$stamp" #Name of outputted file.
	dest_path="/Users/tony/development/scripts/archives" #Absolute path for recordings. No trailing slash!
